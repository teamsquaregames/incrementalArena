Shader "Hidden/nickeltin/SDF/SDFGenerator"
{
    Properties
    {
        ...
        _Channel("Channel", Int) = 3
//        _SolidThreshold("Solid Threshold", Float) = 0.5
    }
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            fixed4 frag(v2f input) : SV_Target
            {
                ...
                for (float i = 0; i < 8; ++i)
                {
                    float2 neighbourUV = uv + offsetedUV * neighbours8offsets[i];
                    float4 edge = tex2Dlod(_MainTex, float4(neighbourUV, 0, 0)) * inBounds(neighbourUV);
                    ...
                    if ((edge[_Channel] >= 0.5) != selfSolid)
                    {
                        ...
                        neighbourUV = lerp(uv, neighbourUV, l);
                        if (length((neighbourUV.xy - uv) * dstFact) < length((outuv.xy - uv) * dstFact))
                        {
                            outuv.xy = neighbourUV.xy;
                ...
                for (float i = 0; i < 8; ++i)
                {
                    ...
                    float2 neighbourUV = uv + offsetedUV * neighbours8offsets[i];
                    float4 edge = sampleEdgeUV(neighbourUV);
        ...
        Pass
        {
            ...
            fixed4 frag(v2f input) : SV_Target
            {
                ...
                float4 source = 0;// tex2D(_SourceTex, edge.b > 0.5 ? input.uv : edge.xy);
                switch (_Channel)
                {
                    case 0: source.r = dst;
                        break;
